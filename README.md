...
音威子府村の「ねっぷちゃん」にインスパイアされた、小平町の**非公式**おしゃべりボット。
スクールバスアプリとは独立したスタンドアロン機能で、**ログイン不要**で
`/obira-chan` にアクセスすると町のことをユーモアを交えて答えます。

- `ANTHROPIC_API_KEY` を環境変数に設定すると Claude API（`claude-opus-4-8`）で会話します
- 未設定でもキーワードベースのローカル応答で動作します（APIキー不要・追加費用なし）
- 公開APIのため、IP単位の簡易レート制限（10回/分）を入れています
- あくまで非公式のため、UI上でも公式情報への確認を促す注意書きを表示しています
